...
class TestCustomTransformers:
    ...
    def test_missing_values_handler(self):
        ...
        handler = MissingValuesHandler(
            none_features=[], 
            zero_features=['GarageArea'], 
            group_impute={}, 
            mode_features=['GarageType']
        )
        handler.fit(df)
    ...
    def test_skewness_corrector(self):
        ...
        df = pd.DataFrame({
            'SkewedFeature': skewed_data,
            'NormalFeature': normal_data,
            'BinaryFeature': [0, 1] * 50  # Devrait être ignoré
        })
        ...
        np.testing.assert_array_almost_equal(
            df_transformed['SkewedFeature'], 
            np.log1p(df['SkewedFeature'])
        )
