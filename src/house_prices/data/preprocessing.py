    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        if all(col in X_copy.columns for col in bathroom_cols):
            X_copy['TotalBathrooms'] = (
                X_copy['FullBath'] + 0.5 * X_copy['HalfBath'] +
                X_copy['BsmtFullBath'].fillna(0) + 0.5 * X_copy['BsmtHalfBath'].fillna(0)
            )
        ...
        if 'GarageArea' in X_copy.columns and 'GarageCars' in X_copy.columns:
            X_copy['GarageAreaPerCar'] = np.where(
                X_copy['GarageCars'] > 0,
                X_copy['GarageArea'] / X_copy['GarageCars'],
                0
            )
        
        # Feature 7: Surface habitable par pièce
        if 'GrLivArea' in X_copy.columns and 'TotRmsAbvGrd' in X_copy.columns:
            X_copy['AreaPerRoom'] = np.where(
                X_copy['TotRmsAbvGrd'] > 0,
                X_copy['GrLivArea'] / X_copy['TotRmsAbvGrd'],
                0
            )
